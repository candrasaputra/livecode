...
function findEmptySeat(arr, row) {
  ...
  if (indexRow > -1) {
    for (let i = 1; i < arr[indexRow].length; i++) {
      if (arr[indexRow][i] === ' ') {
        totoalKosong++
        arrKosong.push(row + arr[0][i]);
      }
    }
  }

  return (totoalKosong === 0) ? `Bangku di baris ${row} sudah penuh` : `Terdapat ${totoalKosong} bangku kosong, ${arrKosong}`;
}
